...
class AuditViewSet(viewsets.ViewSet):
  ...
  def summary(self, request):
    ...
    for ind in indicators:
      # Note: This is an expensive N+1, but for personal use (118 items) it's fine.
      # Ideally compute_due_status should be optimized or fields denormalized.
      st = compute_due_status(ind)[2]
      counts[st] = counts.get(st,0)+1
      if st in ["OVERDUE", "NOT_STARTED"] and len(gap_list) < 25:
          gap_list.append({
              "id": str(ind.id),
    ...
    return Response({"period":period,"start":start_date,"end":end_date,"counts":counts, "gap_list": gap_list})
